...
color_t shade(light_t *light, object_t *object, dmatrix_t *e, dmatrix_t *d, color_t color, color_t background) {
    ...
    dmatrix_t *Te, *Td, *Ts, *S, *V, *R, *I, *N;
    color.r = 0.0; color.g =0.0; color.b = 0.0;
    for (k = 0; k < nobjects; k++) {
        Te = dmat_mult(&object[k].Minv, e);
        Td = dmat_mult(&object[k].Minv, d);
        t0[k] = (object[k].intersection_function)(Te,Td); //computes the intersection
          }
    ...
    if (h != -1){ 
        ...
        R = vector_to_specular_reflection(N, S); 
        ...
        if (Id < 0.0) Id = 0.0;  
        if (Is < 0.0) Is = 0.0; else if (Is > 0.0) Is = pow(Is, object[h].f);
        
              
       //color red
       color.r = MAX_INTENSITY*((object[h].ambient_coeff*object[h].ambient_color.r) + light->color.r*light->intensity.r*((Id*object[h].diffuse_coeff*object[h].diffuse_color.r)+(Is*object[h].specular_coeff*object[h].specular_color.r)));
       //color green
       color.g = MAX_INTENSITY*((object[h].ambient_coeff*object[h].ambient_color.g) + light->color.g*light->intensity.g*((Id*object[h].diffuse_coeff*object[h].diffuse_color.g)+(Is*object[h].specular_coeff*object[h].specular_color.g)));
       //color blue
       color.b = MAX_INTENSITY*((object[h].ambient_coeff*object[h].ambient_color.b) + light->color.b*light->intensity.b*((Id*object[h].diffuse_coeff*object[h].diffuse_color.b)+(Is*object[h].specular_coeff*object[h].specular_color.b)));
